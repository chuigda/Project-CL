...
#define CC_LEADING_ZEROS_IMPL(SUFFIX, TARGET_TYPE, BASE_TYPE, BUILTIN)         \
  static inline unsigned int cc_leading_zeros_##SUFFIX(TARGET_TYPE x) {        \
...
CC_LEADING_ZEROS_IMPL(ushort, unsigned short, unsigned int, __builtin_clz)

CC_LEADING_ZEROS_IMPL(uint, unsigned int, unsigned int, __builtin_clz)

CC_LEADING_ZEROS_IMPL(ulong, unsigned long, unsigned long, __builtin_clzl)

CC_LEADING_ZEROS_IMPL(ulonglong, unsigned long long, unsigned long long,
                      __builtin_clzll)
...
#define CC_TRAILING_ZEROS_IMPL(SUFFIX, TARGET_TYPE, BASE_TYPE, BUILTIN)        \
  static inline unsigned int cc_trailing_zeros_##SUFFIX(TARGET_TYPE x) {       \
    _Static_assert(sizeof(BASE_TYPE) >= sizeof(TARGET_TYPE), #BASE_TYPE        \
                   " must be wider than or of equal size with " #BASE_TYPE);   \
...
CC_TRAILING_ZEROS_IMPL(ushort, unsigned short, unsigned int, __builtin_ctz)

CC_TRAILING_ZEROS_IMPL(uint, unsigned int, unsigned int, __builtin_ctz)

CC_TRAILING_ZEROS_IMPL(ulong, unsigned long, unsigned long, __builtin_ctzl)

CC_TRAILING_ZEROS_IMPL(ulonglong, unsigned long long, unsigned long long,
                       __builtin_ctzll)
