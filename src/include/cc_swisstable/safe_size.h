...
static inline
cc_size cc_st_max_safe_size(void) {
    return ((cc_size) ((cc_isize) -1) >> 1U);
}

static inline
cc_st_safe_size cc_st_safe_size_create(cc_size size) {
    return CC_UNLIKELY(size <= cc_st_max_safe_size()) ?
           (cc_st_safe_size) size : -1;
}

static inline
_Bool cc_st_safe_size_is_valid(cc_st_safe_size size) {
    return size >= 0;
}

static inline
cc_st_safe_size cc_st_safe_size_add(cc_st_safe_size a, cc_st_safe_size b) {
...
static inline
cc_st_safe_size cc_st_safe_size_mul(cc_st_safe_size a, cc_st_safe_size b) {
    if (CC_UNLIKELY(a < 0 || b < 0)) {
        return -1;
    }
