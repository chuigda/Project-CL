...
static inline cc_st_group cc_st_as_little_endian(cc_st_group group) {
  ...
  if (PROJECT_CL_IS_BIG_ENDIAN()) {
    for (cc_size i = 0; i < sizeof(cc_st_group) / 2; i++) {
      ...
      group_union.bytes[i] = group_union.bytes[sizeof(cc_st_group) - i - 1];
...
static inline cc_st_bitmask_iter cc_st_group_match_ctrl(cc_st_group group,
  ...
  cc_st_bitmask result = cc_st_as_little_endian((cmp - cc_st_repeat(0x01U)) &
                                                ~cmp & cc_st_repeat(0x80U));
  iter.mask = result;
  return iter;
}

static inline cc_st_bitmask cc_st_group_mask_empty(cc_st_group group) {
  return cc_st_as_little_endian(group & (group << 1) & cc_st_repeat(0x80U));
}
