...
#define cc_hashmap_alignment() \
    (key_layout.alignment > value_layout.alignment ? key_layout.alignment : value_layout.alignment)

#define cc_hashmap_value_offset() \
    (key_layout.size + ((-key_layout.size) & (value_layout.alignment - 1)))
...
cc_hashmap_with_capacity(
    ...
    hashmap->table = cc_swisstable_with_capacity(size, alignment, capacity);
...
static inline
void *cc_hashmap_insert_unchecked(cc_hashmap *map, void *key, void *value) {
    cc_hashmap_load_layout(map);
...
static inline
cc_swisstable_hasher cc_hashmap_hasher_(const cc_hashmap *map) {
    return (cc_swisstable_hasher) {
            .hash = map->hash,
            .state = map->randomness
    };
}
...
void *cc_hashmap_find(const cc_hashmap *map, void *key) {
    return cc_swisstable_find(&map->table, key, cc_hashmap_hasher_(map), map->equal);
}

void cc_hashmap_erase(cc_hashmap *map, void *key) {
    void *slot = cc_swisstable_find(&map->table, key, cc_hashmap_hasher_(map), map->equal);
    if (slot) {
...
_Bool cc_hashmap_reserve(cc_hashmap *map, cc_size additional) {
    return cc_swisstable_reserve(&map->table, additional, cc_hashmap_hasher_(map));
}

void cc_hashmap_visit(cc_hashmap *map, cc_hashmap_visitor *visitor, void *context) {
    cc_hashmap_load_layout(map);
    cc_swisstable_iter iter = cc_swisstable_create_iter(&map->table);
